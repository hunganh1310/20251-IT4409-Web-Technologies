...
class UserNotificationSettings(BaseModel):
        ...
        def summary(self) -> str:
            return f"Email: {self.email_notifications}, Push: {self.push_notifications}, SMS: {self.sms_notifications}"
...
class UserPrivacySettings(BaseModel):
        ...
        def is_private(self) -> bool:
            return not self.profile_visible or not self.allow_friend_requests
...
class UserThemeSettings(BaseModel):
        ...
        border_radius: Optional[int] = Field(default=4)
        compact_mode: bool = Field(default=False)
        transition_speed: Optional[str] = Field(default="normal")

        def palette_summary(self) -> str:
            return f"Palette: {self.custom_palette}, Accent: {self.accent_color}"
    ...
    custom_palette: Optional[dict] = Field(default_factory=dict)
...
class UserLanguageSettings(BaseModel):
        ...
        def language_summary(self) -> str:
            return f"Lang: {self.language}, Region: {self.region}, Fallback: {self.fallback_languages}"
    language: str = Field(default="en")
    region: Optional[str] = Field(default=None)
    fallback_languages: Optional[list] = Field(default_factory=list)
    date_format: Optional[str] = Field(default="YYYY-MM-DD")
    time_format: Optional[str] = Field(default="24h")
...
class UserSecuritySettings(BaseModel):
    ...
    security_questions: Optional[List[str]] = Field(default_factory=list)

    def is_strong_security(self) -> bool:
        return self.two_factor_enabled and self.backup_codes_enabled and self.password_expiry_days <= 90
